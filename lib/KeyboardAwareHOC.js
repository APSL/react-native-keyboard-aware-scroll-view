...
function KeyboardAwareHOC(
  ...
  const AnimatedScrollableComponent = Animated.createAnimatedComponent(ScrollableComponent);
  ...
  return class
    ...
    static propTypes = {
      ...
      useKeyboardDistance: PropTypes.bool,
    ...
    constructor(props: KeyboardAwareHOCProps) {
      ...
      this.state = {
        keyboardSpace: new Animated.Value(keyboardSpace),
      };
    }
    ...
    componentDidMount() {
      ...
      if (Platform.OS === 'ios') {
        ...
        this.keyboardWillShowEvent = Keyboard.addListener(
          'keyboardDidShow',
          throttle(this._updateKeyboardSpace, 250),
        );
        this.keyboardWillHideEvent = Keyboard.addListener(
          'keyboardDidHide',
          throttle(this._resetKeyboardSpace, 250),
        );
      }
    ...
    componentDidUpdate(prevProps: KeyboardAwareHOCProps) {
      if (this.props.viewIsInsideTabBar !== prevProps.viewIsInsideTabBar) {
        ...
        if (this.state.keyboardSpace !== keyboardSpace) {
          this._animateKeyboardSpace(keyboardSpace);
        }
      }
    }
    ...
    _animateKeyboardSpace = (toValue, extraScrollHeight, position) => {
      Animated.timing(this.state.keyboardSpace, {
        toValue,
        duration: 200,
      }).start(() => {
        setTimeout(() => {
          this.props.onUpdate && this.props.onUpdate(toValue);
          ...
          } else if (extraScrollHeight) {
            this.scrollForExtraHeightOnAndroid(extraScrollHeight)
          } else if (position) {
            this.scrollToPosition(0, position, true);
          }
        }, 1);
      });
    };
                ...
                (x: number, y: number, width: number, height: number) => {
                  ...
                  if (Platform.OS === 'ios') {
                    if (
                      ...
                      this._animateKeyboardSpace(
                        this.props.useKeyboardDistance
                    ...
                    if (textInputBottomPosition > keyboardPosition) {
                      this._animateKeyboardSpace(
                        frames.endCoordinates.height,
                        frames.tag
                          ? (textInputBottomPosition - keyboardPosition) + 75
                          : totalExtraHeight,
                      );
                    } else if (
                      textInputBottomPosition >
    ...
    _resetKeyboardSpace = () => {
      ...
      if (this.props.enableResetScrollToCoords === false) {
        ...
        if (this.defaultResetScrollToCoords) {
          ...
          this._animateKeyboardSpace(keyboardSpace, null, 0);
        }
      }
    };
    ...
    render() {
      const { contentContainerStyle, onScroll, children, contentBackgroundColor } = this.props;
